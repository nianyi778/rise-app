...
export interface ShareCardParams {
  canvasId: string      // canvas 组件的 id（type="2d"）
  pageInstance: WechatMiniprogram.Page.TrivialInstance
  goalTitle: string     // 目标名，如 "副业启动"
  action: string        // 今日行动，如 "整理 3 个可以接的设计订单方向"
  dayIndex: number      // 第几天，如 12
  streakDays: number    // 连击天数，如 5
  phaseProgress: number // 阶段进度 0~1，如 0.65
  phaseName: string     // 阶段名，如 "建立接单能力"
  nickname: string      // 用户昵称，如 "李凯"
  focusMin?: number     // 专注分钟数，如 25（可选，默认25）
}
 ...
 * 内部坐标系统一使用 px（不带 rpx）。
 */
export async function drawShareCard(params: ShareCardParams): Promise<string> {
  const {
    canvasId,
    pageInstance,
    goalTitle,
  ...
  const canvas: WechatMiniprogram.Canvas = await new Promise((resolve, reject) => {
    const query = pageInstance.createSelectorQuery()
    query
      .select(`#${canvasId}`)
      .fields({ node: true, size: true })
      .exec((res: Array<{node: WechatMiniprogram.Canvas; width: number; height: number}>) => {
  ...
  // 设备像素比，用于高清渲染
  const dpr = wx.getWindowInfo?.().pixelRatio ?? wx.getSystemInfoSync().pixelRatio ?? 2
    ...
    wx.canvasToTempFilePath(
      {
        ...
        fileType: 'jpg',
        quality: 0.95,
        success: (res: WechatMiniprogram.CanvasToTempFilePathSuccessCallbackResult) => resolve(res.tempFilePath),
        fail: (err: WechatMiniprogram.GeneralCallbackResult) => reject(new Error(`canvasToTempFilePath failed: ${JSON.stringify(err)}`)),
      },
    )
  })
}
