...
exports.mockWxLogin = mockWxLogin;
exports.mockCreateGoal = mockCreateGoal;
exports.mockGetGoals = mockGetGoals;
exports.mockGetTodaySession = mockGetTodaySession;
exports.mockGetTodayCheckin = mockGetTodayCheckin;
exports.mockSubmitCheckin = mockSubmitCheckin;
exports.mockStreamChatMessage = mockStreamChatMessage;
exports.mockGetWeeklyReport = mockGetWeeklyReport;
exports.mockUpdateGoalStatus = mockUpdateGoalStatus;
// ── 固定的 mock 实体 ──────────────────────────────────────────────────────────
const MOCK_USER = {
    _id: 'mock_user_001',
    nickname: '李凯',
...
async function mockWxLogin() {
    await delay(300);
    return { token: 'mock_token_abc123', user: Object.assign({}, MOCK_USER) };
}
async function mockCreateGoal(payload) {
...
async function mockGetTodaySession(goalId) {
    await delay(200);
    if (!mockGoal) {
...
async function mockGetTodayCheckin() {
    await delay(200);
    return [];
}
async function mockSubmitCheckin(payload) {
    await delay(400);
    mockStreak += 1;
    const checkin = {
...
function mockStreamChatMessage(_goalId, _checkinId, messages, onChunk, onDone, _onError) {
    var _a, _b;
    const lastMsg = (_b = (_a = messages[messages.length - 1]) === null || _a === void 0 ? void 0 : _a.content) !== null && _b !== void 0 ? _b : '';
    let reply = '我听到你了。';
...
async function mockGetWeeklyReport(_weekOffset = 0) {
    ...
    const statuses = ['done', 'done', 'skipped', 'done', 'done', 'done', 'pending'];
    const moods = ['ok', 'good', null, 'good', 'great', 'great', null];
    return {
...
async function mockUpdateGoalStatus(_goalId, _status) {
    await delay(200);
    return mockGoal !== null && mockGoal !== void 0 ? mockGoal : buildGoal({ rawInput: '把副业做起来', category: 'work', blocker: 'no_time', availableTime: '21:00', dailyMinutes: 25 });
}
// ── 工具 ──────────────────────────────────────────────────────────────────────
function delay(ms) {
